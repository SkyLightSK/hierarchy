...
$(document).ready(function () {
   ...
   // UserList js
    ...
    $('.u-subordinates').on('click' ,function loadSubordinates() {

        var user_id = $(this).parent().find('.dd-content').attr('data-user-id');
        var position_id = $(this).parent().find('.dd-content').attr('data-position-id');
        var user_position = $(this).parent().find('.dd-content').attr('data-position');
        ...
        $.ajax({
            ...
            success:function(data){
                that.parent().append(data.msg);
                that.removeClass('u-subordinates');
                that.off('click');

                updateOutput($('.hierarchy').data('output', $('#nestable-output')));

                //recycle
                $('.u-subordinates').on('click' ,function () {
    ...
    var updateOutput = function(e) {
        ...
        if(window.JSON) {
            output.val(window.JSON.stringify(list.nestable('serialize'), undefined, 4));//, null, 2));
        }
        else {
    ...
    $('.hierarchy').nestable({
        ...
        beforeDragStop: function(l,e, p){
            ...
            var place_position = p.attr('data-level-position');
            var el_user_id = e.attr('data-id');
            var el_user_position_id = e.attr('data-position-id');
            var new_parent_id = p.parent().attr('data-id');
            var new_parent_position_id = p.parent().attr('data-position-id');

            e.attr('data-position' , place_position);

            if(place_position) {
                e.find('.position-text:first').html(' Position:' + place_position);
            }
            ...
            $.ajax({
                ...
                data: {
                    ...
                    child_id : el_user_id,
                    child_position_id : el_user_position_id,
                    new_parent_id : new_parent_id,
                    new_parent_position_id : new_parent_position_id
                },
                type:'POST',
                url:'/updateHierarchy',
